...
DROP TABLE av_treatment_table_1320_4p8 PURGE;
...
-- This SQL code creates a table for cancer diagnoses between 2013 and 2020 with flags if a treatment has occurred wihtin the defined timeframes, and surgical procedure list.
...
--5. For use of the final table, refer to av_treatment_table_YYYY_4pX e.g. av_treatment_table_1320_4p8
...
(SELECT ats.patientid, ats.tumourid, ats.diagnosisdatebest, ats.diagnosisyear, avt.nhsnumber, avt.figo, avt.sex, avt.ethnicity, avt.morph_icd10_o2, avt.fiveyearageband, avt.age, avt.dedup_flag, avt.site_icd10_o2, avt.site_icd10_o2_3char, avt.ctry_code, avt.statusofregistration   
...
END AS tumour_code
FROM __.at_tumour_skin@casref01 ats
LEFT JOIN av2020.at_tumour_england@casref01 avt ON ats.tumourid=avt.tumourid
WHERE ats.diagnosisyear between 2013 and 2020
...
non_skin AS
(SELECT tumourid, patientid, nhsnumber, diagnosisdatebest, site_icd10_o2, figo, sex, ethnicity, morph_icd10_o2, fiveyearageband, age

--Create amended tumour_code variable to differentiate between different types of cancers within ICD10-defined sites.  
,CASE 
WHEN avt.site_icd10_o2_3char IN ('C48') 
...
AND avt.sex=2)
THEN 'C48OVARY' 
WHEN avt.site_icd10_o2_3char IN ('C48') THEN 'C48OTHER' 
WHEN avt.site_icd10_o2 IN ('D391') THEN 'D39OVARY'
WHEN avt.site_icd10_o2_3char = 'D39' AND avt.site_icd10_o2 NOT IN ('D391') THEN 'D39OTHER'
WHEN avt.site_icd10_o2 IN ('D292') THEN 'D29TESTES'
WHEN avt.site_icd10_o2_3char = 'D29' AND avt.site_icd10_o2 NOT IN ('D292') THEN 'D29OTHER'
WHEN avt.site_icd10_o2 IN ('C751','C752','C753') THEN  'C75BRAIN'
WHEN avt.site_icd10_o2_3char = 'C75' AND avt.site_icd10_o2 NOT IN ('C751','C752','C753') THEN 'C75OTHER'
WHEN avt.site_icd10_o2 IN ('D320','D321','D329') THEN 'D32BRAIN'                                                     
WHEN avt.site_icd10_o2 IN ('D330','D331','D332','D333','D334','D337','D339') THEN 'D33BRAIN'                                                             
WHEN avt.site_icd10_o2 IN ('D352','D353','D354') THEN 'D35BRAIN'                                                     
WHEN avt.site_icd10_o2 IN ('D420','D421','D429') THEN 'D42BRAIN'                                                     
WHEN avt.site_icd10_o2 IN ('D430','D431','D432','D433','D434','D437','D439') THEN 'D43BRAIN'                                                             
WHEN avt.site_icd10_o2 IN ('D443','D444','D445') THEN 'D44BRAIN' 
WHEN avt.site_icd10_o2 IN ('D414') THEN 'D41BLADDER'                                                             
WHEN avt.site_icd10_o2 IN ('D090') THEN 'D09BLADDER' 
ELSE avt.site_icd10_o2_3char 
END AS tumour_code

FROM av2020.at_tumour_england@casref01 AVT 

--Define cohort of interest here
WHERE avt.diagnosisyear between 2013 and 2020 
AND avt.site_icd10_o2_3char NOT IN ('D01','D03','D04','D06','D07','D11','D13','D15','D16','D18','D25','D27','D36','D40','D48','C44') 
AND avt.cascade_inci_flag = 1
AND avt.ctry_code = 'E'
AND avt.statusofregistration = 'F'
AND avt.dedup_flag = '1'
AND avt.age BETWEEN 0 AND 200 
...
non_skin_cohort AS
(SELECT nsk.tumourid, nsk.patientid, nsk.nhsnumber, nsk.diagnosisdatebest, nsk.site_icd10_o2, nsk.figo, nsk.sex, nsk.ethnicity, nsk.morph_icd10_o2, nsk.fiveyearageband, nsk.age, nsk.tumour_code
FROM non_skin nsk
...
tumour_cohort AS
(SELECT tumourid, patientid, nhsnumber, diagnosisdatebest, site_icd10_o2, figo, sex, ethnicity, morph_icd10_o2, fiveyearageband, age, tumour_code
FROM skin_cohort
...
SELECT tumourid, patientid, nhsnumber, diagnosisdatebest, site_icd10_o2, figo, sex, ethnicity, morph_icd10_o2, fiveyearageband, age, tumour_code, tumour_flag
FROM 
(SELECT avt.tumourid, avt.patientid, avt.nhsnumber, avt.diagnosisdatebest, avt.site_icd10_o2, avt.figo, avt.sex, avt.ethnicity, avt.morph_icd10_o2, avt.fiveyearageband, avt.age, avt.tumour_code

-- This join flags any tumours diagnosed in 2013-20 that belong to a patient who had another tumour in the 18 months before or after that diagnosis 
...
-- For tumours diagnosed from 2013-2020, identify any other tumour IDs that occurred between 2011-2022 
-- A second copy of the tumour cohort (AVT2) is joined to the original tumour cohort of 2013-20 diagnoses (TC) 
-- Records from AVT2 are only joined if the patient ID is the same but the tumour ID is different 

LEFT JOIN av2020.at_tumour_england@casref01 AVT2 ON avt.patientid=avt2.patientid 
AND NOT(avt.tumourid=avt2.tumourid) 
--AND avt2.cascade_inci_flag = 1 
AND avt2.site_icd10_o2_3char NOT IN ('D01','D03','D04','D06','D07','D11','D13','D15','D16','D18','D25','D27','D36','D40','D48','C44')    
AND avt2.diagnosisyear BETWEEN 2011 AND 2022
...
CREATE UNIQUE INDEX username.tr_tumcohort_tumourid_uq ON username.tr_tumour_cohort ( tumourid ) NOLOGGING TABLESPACE analysisdata_IX; 
CREATE INDEX username.tr_tumcohort_patientid_ix ON username.tr_tumour_cohort ( patientid ) NOLOGGING TABLESPACE analysisdata_IX; 
CREATE INDEX username.tr_tumcohort_nhsnumber_ix ON username.tr_tumour_cohort ( nhsnumber ) NOLOGGING TABLESPACE analysisdata_IX; 
EXECUTE dbms_stats.gather_table_stats('username', 'tr_tumour_cohort') 
EXECUTE dbms_stats.gather_index_stats('username', 'tr_tumcohort_tumourid_uq') 
...
INNER JOIN username.timeframe_lookup_13_20@casref01  tim ON tim.tumouricdsite3code = tc.tumour_code 
INNER JOIN av2020.at_treatment_england@casref01 avtreat ON avtreat.tumourid=tc.tumourid 
AND eventcode IN ('01a','01b','01z', '01c') AND (avtreat.eventdate-tc.diagnosisdatebest BETWEEN -31 AND tim.resect_time) 
INNER JOIN username.opcs4resection_lookup_13_20@casref01 opcs ON opcs.tumouricdsite3code = tc.tumour_code AND TRIM(opcs.opcsresectioncode) = avtreat.opcs4_code 
)
WHERE rk=1
));
...
-- Create a surgery flag for the tumour if: 
-- There is an inpatient hes episode with a tumour resection opcs-4 code in one of the operation fields 
...
INNER JOIN username.opcs4resection_lookup_13_20@casref01 opcs ON opcs.tumouricdsite3code = tc.tumour_code AND TRIM(opcs.opcsresectioncode) = ho.opertn 
) 
WHERE rk=1)
...
INNER JOIN username.opcs4resection_lookup_13_20@casref01 opcs ON opcs.tumouricdsite3code = tc.tumour_code
...
------------------------------------------------------------------------------ 
--3)---------------- LIVER C22 - AT_TREATMENT_ENGLAND ------------------ 
-- Create a surgery flag for the tumour if: 
-- there is a record in AT_TREATMENT_ENGLAND which states that the tumour was treated with surgery (event is '01a', '01b', '01z', or '01c') 
-- and the opcs4_code is a percutaneous radiofrequency AND microwave ablation of lesion of liver (see SOP Appendices for list of opcs4 codes) 
-- and the operation date (opertn) occurred in the relevant timeframe (see SOP) 
...
INNER JOIN av2020.at_treatment_england@casref01 avtreat ON avtreat.tumourid=tc.tumourid 
AND eventcode IN ('01a','01b','01z','01c') AND (avtreat.eventdate-tc.diagnosisdatebest BETWEEN -31 AND tim.resect_time) 
AND avtreat.opcs4_code IN ('J124','J127') AND tc.tumour_code IN ('C22')) 
WHERE rk=1));
...
INNER JOIN heslive.hesapc_opertn@casref01 ho ON ho.datayear = hl.datayear AND ho.epikeyanon = hl.epikeyanon 
AND ho.opdate-tc.diagnosisdatebest BETWEEN -31 AND tim.resect_time 
AND ho.opertn IN ('J124','J127') AND tc.tumour_code in ('C22')) 
WHERE rk=1)
...
INNER JOIN heslive.hesop_opertn@casref01 ho2 ON ho2.datayear = h2.datayear AND ho2.attendkeyanon = h2.attendkeyanon
where op.apptdate-tc.diagnosisdatebest BETWEEN -31 AND tim.resect_time 
AND ho2.opertn IN ('J124','J127') AND tc.tumour_code in ('C22')) 
WHERE rk=1)))
WHERE rk=1); 
...
INNER JOIN av2020.at_treatment_england@casref01 avtreat ON avtreat.tumourid=tc.tumourid 
AND eventcode IN ('01a','01b','01z','01c') AND (avtreat.eventdate-tc.diagnosisdatebest BETWEEN -31 AND tim.resect_time) 
AND avtreat.opcs4_code IN ('H024','H019','H011') AND tc.site_icd10_o2 in ('C181')) 
WHERE rk=1)); 
...
INNER JOIN heslive.hesapc_opertn@casref01 ho ON ho.datayear = hl.datayear AND ho.epikeyanon = hl.epikeyanon 
AND ho.opdate-tc.diagnosisdatebest BETWEEN -31 AND tim.resect_time 
AND ho.opertn IN ('H024','H019','H011') AND tc.site_icd10_o2 in ('C181')) 
WHERE rk=1)
...
INNER JOIN heslive.hesop_opertn@casref01 ho2 ON ho2.datayear = h2.datayear AND ho2.attendkeyanon = h2.attendkeyanon
where op.apptdate-tc.diagnosisdatebest BETWEEN -31 AND tim.resect_time 
AND ho2.opertn IN ('H024','H019','H011') AND tc.site_icd10_o2 in ('C181')) 
WHERE rk=1)))
WHERE rk=1);
...
tumourid, datediff ,rk, start_date_of_regimen, sact2_trust_code 
	FROM (SELECT tc.tumourid, 
		...
		FROM tr_tumour_cohort tc 
		INNER JOIN username.timeframe_lookup_13_20@casref01 TIM ON TIM.tumouricdsite3code = tc.tumour_code 
		INNER JOIN sact.at_patient_england@cas2303 sp ON tc.nhsnumber=sp.nhs_number 
		INNER JOIN sact.at_tumour_england@cas2303 st ON sp.encore_patient_id = st.encore_patient_id
		INNER JOIN sact.at_regimen_england@cas2303 sr ON st.sact_tumour_id=sr.sact_tumour_id 
		AND (NOT (benchmark_group IN ('NOT CHEMO','HORMONES','ZOLEDRONIC ACID','PAMIDRONATE','DENOSUMAB', 'RADIUM 223', 'LUTETIUM-177', 'YTTRIUM-90') OR benchmark_group IS NULL)) 
		AND sr.start_date_of_regimen-tc.diagnosisdatebest BETWEEN -31 AND TIM.chemo_time 
...
------------------------------------------------------------------------------ 
--22)---------------- ALL SITES - AT_TREATMENT_ENGLAND ------------------ 
...
--23)---------------ALL SITES - RTDS PRE APRIL 2016 (COLLECTED BY NATCANSAT)----------- 
-- Create a radiotherapy flag for the tumour if: 
...
--24)--------------- ALL SITES - RTDS POST APRIL 2016 (COLLECTED BY NCRAS; PROCESSED BY ENCORE) ----------------- 
-- Create a radiotherapy flag for the tumour if: 
-- There is a record in rtds (excluding those classed as Brachytherapy, i.e., with RTTREATMENTMODALITY='06') -- removed this restriction due to now counting radio-isotope treatments as radiotherapy rather than chemotherapy
-- AND the appointment date (APPTDATE) occurred in the relevant timeframe 
-- AND the patient only had one tumour in the time period of interest (this is also incorporated in the final table) 
...
FROM tr_tumour_cohort tc 
INNER JOIN username.timeframe_lookup_13_20@casref01  tim ON tim.tumouricdsite3code = tc.tumour_code 
INNER JOIN rtds.at_prescriptions_england@cas2303 pr ON pr.patientid=tc.patientid
...
------------------------------------------------------------------------------- 
CREATE UNIQUE INDEX username.tr_AVCT_tumourid_uq ON username.tr_av_CT ( tumourid ) NOLOGGING TABLESPACE analysisdata_IX; 
CREATE UNIQUE INDEX username.tr_AVRT_tumourid_uq ON username.tr_av_RT ( tumourid ) NOLOGGING TABLESPACE analysisdata_IX; 
CREATE UNIQUE INDEX username.tr_AVSG_tumourid_uq ON username.tr_av_sg ( tumourid ) NOLOGGING TABLESPACE analysisdata_IX; 
CREATE UNIQUE INDEX username.tr_av_bladder_tumourid_uq ON username.tr_av_bladder ( tumourid ) NOLOGGING TABLESPACE analysisdata_IX; 
CREATE UNIQUE INDEX username.tr_av_coloappen_tumourid_uq ON username.tr_av_coloappen ( tumourid ) NOLOGGING TABLESPACE analysisdata_IX; 
CREATE UNIQUE INDEX username.tr_av_colorec_tumourid_uq ON username.tr_av_colorec ( tumourid ) NOLOGGING TABLESPACE analysisdata_IX; 
CREATE UNIQUE INDEX username.tr_av_conebiops_tumourid_uq ON username.tr_av_conebiops ( tumourid ) NOLOGGING TABLESPACE analysisdata_IX; 
CREATE UNIQUE INDEX username.tr_av_liver_tumourid_uq ON username.tr_av_liver ( tumourid ) NOLOGGING TABLESPACE analysisdata_IX; 
CREATE UNIQUE INDEX username.tr_av_lymph_tumourid_uq ON username.tr_av_lymph ( tumourid ) NOLOGGING TABLESPACE analysisdata_IX; 
CREATE UNIQUE INDEX username.tr_av_oesoph_tumourid_uq ON username.tr_av_oesoph ( tumourid ) NOLOGGING TABLESPACE analysisdata_IX; 
CREATE UNIQUE INDEX username.tr_av_stomach_tumourid_uq ON username.tr_av_stomach ( tumourid ) NOLOGGING TABLESPACE analysisdata_IX;

EXECUTE dbms_stats.gather_table_stats('username', 'tr_av_CT') 
EXECUTE dbms_stats.gather_index_stats('username', 'tr_AVCT_tumourid_uq') 
EXECUTE dbms_stats.gather_table_stats('username', 'tr_av_RT') 
EXECUTE dbms_stats.gather_index_stats('username', 'tr_AVRT_tumourid_uq') 
EXECUTE dbms_stats.gather_table_stats('username', 'tr_av_sg') 
EXECUTE dbms_stats.gather_index_stats('username', 'tr_AVSG_tumourid_uq') 
EXECUTE dbms_stats.gather_table_stats('username', 'tr_av_bladder') 
EXECUTE dbms_stats.gather_index_stats('username', 'tr_av_bladder_tumourid_uq') 
EXECUTE dbms_stats.gather_table_stats('username', 'tr_av_coloappen') 
EXECUTE dbms_stats.gather_index_stats('username', 'tr_av_coloappen_tumourid_uq') 
EXECUTE dbms_stats.gather_table_stats('username', 'tr_av_colorec') 
EXECUTE dbms_stats.gather_index_stats('username', 'tr_av_colorec_tumourid_uq') 
EXECUTE dbms_stats.gather_table_stats('username', 'tr_av_conebiops') 
EXECUTE dbms_stats.gather_index_stats('username', 'tr_av_conebiops_tumourid_uq') 
EXECUTE dbms_stats.gather_table_stats('username', 'tr_av_liver') 
EXECUTE dbms_stats.gather_index_stats('username', 'tr_av_liver_tumourid_uq') 
EXECUTE dbms_stats.gather_table_stats('username', 'tr_av_lymph') 
EXECUTE dbms_stats.gather_index_stats('username', 'tr_av_lymph_tumourid_uq') 
EXECUTE dbms_stats.gather_table_stats('username', 'tr_av_oesoph') 
EXECUTE dbms_stats.gather_index_stats('username', 'tr_av_oesoph_tumourid_uq') 
EXECUTE dbms_stats.gather_table_stats('username', 'tr_av_stomach') 
EXECUTE dbms_stats.gather_index_stats('username', 'tr_av_stomach_tumourid_uq')

CREATE UNIQUE INDEX username.tr_hes_sg_tumourid_uq ON username.tr_hes_sg ( tumourid ) NOLOGGING TABLESPACE analysisdata_IX; 
CREATE UNIQUE INDEX username.tr_hes_bladder_tumid_uq ON username.tr_hes_bladder ( tumourid ) NOLOGGING TABLESPACE analysisdata_IX; 
CREATE UNIQUE INDEX username.tr_hes_coloappen_tumid_uq ON username.tr_hes_coloappen ( tumourid ) NOLOGGING TABLESPACE analysisdata_IX; 
CREATE UNIQUE INDEX username.tr_hes_colorec_tumourid_uq ON username.tr_hes_colorec ( tumourid ) NOLOGGING TABLESPACE analysisdata_IX; 
CREATE UNIQUE INDEX username.tr_hes_conebiops_tumid_uq ON username.tr_hes_conebiops ( tumourid ) NOLOGGING TABLESPACE analysisdata_IX; 
CREATE UNIQUE INDEX username.tr_hes_liver_tumourid_uq ON username.tr_hes_liver ( tumourid ) NOLOGGING TABLESPACE analysisdata_IX; 
CREATE UNIQUE INDEX username.tr_hes_lymph_tumourid_uq ON username.tr_hes_lymph ( tumourid ) NOLOGGING TABLESPACE analysisdata_IX; 
CREATE UNIQUE INDEX username.tr_hes_oesoph_tumourid_uq ON username.tr_hes_oesoph ( tumourid ) NOLOGGING TABLESPACE analysisdata_IX; 
CREATE UNIQUE INDEX username.tr_hes_stomach_tumourid_uq ON username.tr_hes_stomach ( tumourid ) NOLOGGING TABLESPACE analysisdata_IX; 
CREATE UNIQUE INDEX username.tr_rtds_tumourid_uq ON username.tr_rtds ( tumourid ) NOLOGGING TABLESPACE analysisdata_IX; 
CREATE UNIQUE INDEX username.tr_rtds_2_tumourid_uq ON username.tr_rtds_2 ( tumourid ) NOLOGGING TABLESPACE analysisdata_IX; 
CREATE UNIQUE INDEX username.tr_sact_tumourid_uq ON username.tr_sact ( tumourid ) NOLOGGING TABLESPACE analysisdata_IX; 
CREATE UNIQUE INDEX username.tr_sact_2_tumourid_uq ON username.tr_sact_2 ( tumourid ) NOLOGGING TABLESPACE analysisdata_IX; 

EXECUTE dbms_stats.gather_table_stats('username', 'tr_hes_sg') 
EXECUTE dbms_stats.gather_index_stats('username', 'tr_hes_sg_tumourid_uq') 
EXECUTE dbms_stats.gather_table_stats('username', 'tr_hes_bladder') 
EXECUTE dbms_stats.gather_index_stats('username', 'tr_hes_bladder_tumid_uq')
EXECUTE dbms_stats.gather_table_stats('username', 'tr_hes_coloappen') 
EXECUTE dbms_stats.gather_index_stats('username', 'tr_hes_coloappen_tumid_uq') 
EXECUTE dbms_stats.gather_table_stats('username', 'tr_hes_colorec') 
EXECUTE dbms_stats.gather_index_stats('username', 'tr_hes_colorec_tumourid_uq') 
EXECUTE dbms_stats.gather_table_stats('username', 'tr_hes_conebiops') 
EXECUTE dbms_stats.gather_index_stats('username', 'tr_hes_conebiops_tumid_uq') 
EXECUTE dbms_stats.gather_table_stats('username', 'tr_hes_liver') 
EXECUTE dbms_stats.gather_index_stats('username', 'tr_hes_liver_tumourid_uq') 
EXECUTE dbms_stats.gather_table_stats('username', 'tr_hes_lymph') 
EXECUTE dbms_stats.gather_index_stats('username', 'tr_hes_lymph_tumourid_uq') 
EXECUTE dbms_stats.gather_table_stats('username', 'tr_hes_oesoph') 
EXECUTE dbms_stats.gather_index_stats('username', 'tr_hes_oesoph_tumourid_uq') 
EXECUTE dbms_stats.gather_table_stats('username', 'tr_hes_stomach') 
EXECUTE dbms_stats.gather_index_stats('username', 'tr_hes_stomach_tumourid_uq') 
EXECUTE dbms_stats.gather_table_stats('username', 'tr_rtds') 
EXECUTE dbms_stats.gather_index_stats('username', 'tr_rtds_tumourid_uq') 
EXECUTE dbms_stats.gather_table_stats('username', 'tr_rtds_2') 
EXECUTE dbms_stats.gather_index_stats('username', 'tr_rtds_2_tumourid_uq') 
EXECUTE dbms_stats.gather_table_stats('username', 'tr_sact') 
EXECUTE dbms_stats.gather_index_stats('username', 'tr_sact_tumourid_uq') 
EXECUTE dbms_stats.gather_table_stats('username', 'tr_sact_2') 
EXECUTE dbms_stats.gather_index_stats('username', 'tr_sact_2_tumourid_uq') 
 
------------------------------------------------------------------------------ 
----------- Create final table drawing on all previous tables------------------ 
------------------------------------------------------------------------------ 
CREATE TABLE av_treatment_table_1320_4p8 COMPRESS
AS 
SELECT 
...
--Cervical 
WHEN avt.site_icd10_o2_3char='C53' AND (upper(SUBSTR(tc.figo,1,2))) IN ('1A','IA') AND conebiops_avtreat=1 THEN 1 
WHEN avt.site_icd10_o2_3char='C53' AND (upper(SUBSTR(tc.figo,1,2))) IN ('1A','IA') AND conebiops_hes=1 AND tc.tumour_flag=0 THEN 1 
WHEN avt.site_icd10_o2_3char='C53' AND (upper(tc.figo) IN ('1B','IB') or upper(SUBSTR(tc.figo,1,3)) IN ('1B1','IB1')) AND (conebiops_avtreat=1) AND (lymph_avtreat=1) THEN 1 
WHEN avt.site_icd10_o2_3char='C53' AND (upper(tc.figo) IN ('1B','IB') or upper(SUBSTR(tc.figo,1,3)) IN ('1B1','IB1')) AND (conebiops_avtreat=1) AND (lymph_hes=1 AND tc.tumour_flag=0) THEN 1 
WHEN avt.site_icd10_o2_3char='C53' AND (upper(tc.figo) IN ('1B','IB') or upper(SUBSTR(tc.figo,1,3)) IN ('1B1','IB1')) AND (conebiops_hes=1 AND tc.tumour_flag=0) AND (lymph_avtreat=1) THEN 1 
WHEN avt.site_icd10_o2_3char='C53' AND (upper(tc.figo) IN ('1B','IB') or upper(SUBSTR(tc.figo,1,3)) IN ('1B1','IB1')) AND (conebiops_hes=1 AND tc.tumour_flag=0) AND (lymph_hes=1 AND tc.tumour_flag=0) THEN 1 

--colorectal: 
WHEN avt.site_icd10_o2_3char IN ('C18','C19','C20') AND SUBSTR(avt.stage_best,1,1)='1' AND colorec_avtreat=1 THEN 1 
WHEN avt.site_icd10_o2_3char IN ('C18','C19','C20') AND SUBSTR(avt.stage_best,1,1)='1' AND colorec_hes=1 AND tc.tumour_flag=0 THEN 1 
--Sub rule for appendectomies for colorectal: 
WHEN avt.site_icd10_o2 IN ('C181') AND colorec_avtreat_appen=1 THEN 1 
WHEN avt.site_icd10_o2 IN ('C181') AND colorec_hes_appen=1 AND tc.tumour_flag=0 THEN 1 

--bladder 
WHEN avt.site_icd10_o2_3char IN ('C67') AND SUBSTR(avt.t_best, 1,1) = '1' AND bladder_avtreat=1 THEN 1 
WHEN avt.site_icd10_o2_3char IN ('C67') AND SUBSTR(avt.t_best, 1,1) = '1' AND bladder_hes=1 AND tc.tumour_flag=0 THEN 1

WHEN avt.site_icd10_o2 IN ('D090') AND avt.morph_icd10_o2 = '8130' AND bladder_avtreat=1 THEN 1 
WHEN avt.site_icd10_o2 IN ('D090') AND avt.morph_icd10_o2 = '8130' AND bladder_hes=1 AND tc.tumour_flag=0 THEN 1

-- liver 
WHEN avt.site_icd10_o2_3char IN ('C22') AND SUBSTR(avt.stage_best,1,1)='1' AND liver_avtreat=1 THEN 1 
WHEN avt.site_icd10_o2_3char IN ('C22') AND SUBSTR(avt.stage_best,1,1)='1' AND liver_hes=1 AND tc.tumour_flag=0 THEN 1 

-- oesophagus 
WHEN avt.site_icd10_o2_3char IN ('C15') AND SUBSTR(avt.stage_best, 1,2)='1A' AND oesoph_avtreat=1 THEN 1 
WHEN avt.site_icd10_o2_3char IN ('C15') AND SUBSTR(avt.stage_best,1,2)='1A' AND oesoph_hes=1 AND tc.tumour_flag=0 THEN 1 

-- stomach 
WHEN avt.site_icd10_o2_3char IN ('C16') AND SUBSTR(avt.stage_best,1,2)='1A' AND stomach_avtreat=1 THEN 1 
WHEN avt.site_icd10_o2_3char IN ('C16') AND SUBSTR(avt.stage_best,1,2)='1A' AND stomach_hes=1 AND tc.tumour_flag=0 THEN 1 
ELSE 0 
...
WHEN tumour_code IN ('C22') THEN 'LIVER' 
WHEN tumour_code IN ('C33', 'C34') AND tc.morph_icd10_o2 IN ('8041','8042','8043','8044','8045') THEN 'SCLC' 
WHEN tumour_code IN ('C33', 'C34') AND tc.morph_icd10_o2 NOT IN ('8041','8042','8043','8044','8045') THEN 'NSCLC' 
...
,case
    ...
    when (diagnosisyear IN ('2014', '2015', '2016', '2017', '2018', '2019', '2020')) then IMD19_quintile_lsoas
    end as imd_quintile_lsoas
,atg.canalliance_2021_name
,atg.canalliance_2021_code
--For checking 
,avt.morph_icd10_o2 
,tc.figo 
,avt.t_best 
...
, CASE 
WHEN avt.site_icd10_o2_3char='C53' AND (upper(SUBSTR(tc.figo,1,2)) IN ('1A','IA')) AND conebiops_avtreat=1 THEN cbavt.avsg_date 
WHEN avt.site_icd10_o2_3char='C53' AND (upper(tc.figo) IN ('1B','IB') or upper(SUBSTR(tc.figo,1,3)) IN ('1B1','IB1')) AND (conebiops_avtreat=1) AND (lymph_avtreat=1) THEN cbavt.avsg_date 
WHEN avt.site_icd10_o2_3char='C53' AND (upper(tc.figo) IN ('1B','IB') or upper(SUBSTR(tc.figo,1,3)) IN ('1B1','IB1')) AND (conebiops_avtreat=1) AND (lymph_hes=1 AND tc.tumour_flag=0) THEN cbavt.avsg_date 
END AS cbavsg_date 
...
, CASE 
WHEN avt.site_icd10_o2_3char='C53' AND (upper(SUBSTR(tc.figo,1,2)) IN ('1A','IA')) AND conebiops_hes=1 AND tc.tumour_flag=0 THEN cbhes.hessg_date 
WHEN avt.site_icd10_o2_3char='C53' AND (upper(tc.figo) IN ('1B','IB') or upper(SUBSTR(tc.figo,1,3)) IN ('1B1','IB1')) AND (conebiops_hes=1 AND tc.tumour_flag=0) AND (lymph_avtreat=1) THEN cbhes.hessg_date 
WHEN avt.site_icd10_o2_3char='C53' AND (upper(tc.figo) IN ('1B','IB') or upper(SUBSTR(tc.figo,1,3)) IN ('1B1','IB1')) AND (conebiops_hes=1 AND tc.tumour_flag=0) AND (lymph_hes=1 AND tc.tumour_flag=0) THEN cbhes.hessg_date 
END AS cbhessg_date 
...
,CASE WHEN avt.site_icd10_o2_3char IN ('C18','C19','C20') AND SUBSTR(avt.stage_best,1,1)='1' AND colorec_avtreat=1 THEN coloavt.avsg_date 
END AS coloavsg_date 
,CASE WHEN avt.site_icd10_o2_3char IN ('C18','C19','C20') AND SUBSTR(avt.stage_best,1,1)='1' AND colorec_hes=1 AND tc.tumour_flag=0 THEN colohes.hessg_date 
END AS colohessg_date 
,CASE WHEN avt.site_icd10_o2 IN ('C181') AND colorec_avtreat_appen=1 THEN coloavt_appen.avsg_date 
END AS appenavsg_date 
, CASE WHEN avt.site_icd10_o2 IN ('C181') AND colorec_hes_appen=1 AND tc.tumour_flag=0 THEN colohes_appen.hessg_date 
END AS appenhessg_date

---------------bladder--------------------------------- 
,CASE WHEN avt.site_icd10_o2_3char IN ('C67') AND SUBSTR(avt.t_best, 1,1) = '1' AND bladder_avtreat=1 THEN blad1_avt.avsg_date 
END AS bladavsg_date 
, CASE WHEN avt.site_icd10_o2_3char IN ('C67') AND SUBSTR(avt.t_best, 1,1) = '1' AND bladder_hes=1 AND tc.tumour_flag=0 THEN blad1_hes.hessg_date 
END AS bladhessg_date 

,CASE WHEN avt.site_icd10_o2 IN ('D090') AND avt.morph_icd10_o2 = '8130' AND bladder_avtreat=1 THEN blad1_avt.avsg_date 
END AS blad_insitu_avsg_date 
, CASE WHEN avt.site_icd10_o2 IN ('D090') AND avt.morph_icd10_o2 = '8130' AND bladder_hes=1 AND tc.tumour_flag=0 THEN blad1_hes.hessg_date 
END AS blad_insitu_hessg_date 

---------------liver--------------------------------- 
,CASE WHEN avt.site_icd10_o2_3char IN ('C22') AND SUBSTR(avt.stage_best,1,1)='1' AND liver_avtreat=1 THEN livavt.avsg_date 
END AS livavsg_date 
, CASE WHEN avt.site_icd10_o2_3char IN ('C22') AND SUBSTR(avt.stage_best,1,1)='1' AND liver_hes=1 AND tc.tumour_flag=0 THEN livhes.hessg_date 
END AS livhessg_date 

---------------oesophageal--------------------------------- 
,CASE WHEN avt.site_icd10_o2_3char IN ('C15') AND SUBSTR(avt.stage_best,1,2)='1A' AND oesoph_avtreat=1 THEN oesoavt.avsg_date 
END AS oesoavsg_date 
, CASE WHEN avt.site_icd10_o2_3char IN ('C15') AND SUBSTR(avt.stage_best,1,2)='1A' AND oesoph_hes=1 AND tc.tumour_flag=0 THEN oesohes.hessg_date 
END AS oesohessg_date 

---------------stomach--------------------------------- 
, CASE WHEN avt.site_icd10_o2_3char IN ('C16') AND SUBSTR(avt.stage_best,1,2)='1A' AND stomach_avtreat=1 THEN stomavt.avsg_date 
END AS stomavsg_date 
, CASE WHEN avt.site_icd10_o2_3char IN ('C16') AND SUBSTR(avt.stage_best,1,2)='1A' AND stomach_hes=1 AND tc.tumour_flag=0 THEN stomhes.hessg_date 
...
, CASE 
WHEN avt.site_icd10_o2_3char='C53' AND (upper(SUBSTR(tc.figo,1,2)) IN ('1A','IA')) AND conebiops_avtreat=1 THEN cbavt.avsg_trust_code 
WHEN avt.site_icd10_o2_3char='C53' AND (upper(tc.figo) IN ('1B','IB') or upper(SUBSTR(tc.figo,1,3)) IN ('1B1','IB1')) AND (conebiops_avtreat=1) AND (lymph_avtreat=1) THEN cbavt.avsg_trust_code 
WHEN avt.site_icd10_o2_3char='C53' AND (upper(tc.figo) IN ('1B','IB') or upper(SUBSTR(tc.figo,1,3)) IN ('1B1','IB1')) AND (conebiops_avtreat=1) AND (lymph_hes=1 AND tc.tumour_flag=0) THEN cbavt.avsg_trust_code 
...
, CASE 
WHEN avt.site_icd10_o2_3char='C53' AND (upper(SUBSTR(tc.figo,1,2)) IN ('1A','IA')) AND conebiops_hes=1 AND tc.tumour_flag=0 THEN cbhes.hessg_trust_code 
WHEN avt.site_icd10_o2_3char='C53' AND (upper(tc.figo) IN ('1B','IB') or upper(SUBSTR(tc.figo,1,3)) IN ('1B1','IB1')) AND (conebiops_hes=1 AND tc.tumour_flag=0) AND (lymph_avtreat=1) THEN cbhes.hessg_trust_code 
WHEN avt.site_icd10_o2_3char='C53' AND (upper(tc.figo) IN ('1B','IB') or upper(SUBSTR(tc.figo,1,3)) IN ('1B1','IB1')) AND (conebiops_hes=1 AND tc.tumour_flag=0) AND (lymph_hes=1 AND tc.tumour_flag=0) THEN cbhes.hessg_trust_code 
END AS cbhessg_trust_code 

---------------colorectal--------------------------------- 
-- As with cervical, select the date of the stage-specific resection for each tumour, according to the rules specified earlier for generating the stage-specific resection flag for that tumour site 
,CASE WHEN avt.site_icd10_o2_3char IN ('C18','C19','C20') AND SUBSTR(avt.stage_best,1,1)='1' AND colorec_avtreat=1 THEN coloavt.avsg_trust_code 
END AS coloavsg_trust_code 
,CASE WHEN avt.site_icd10_o2_3char IN ('C18','C19','C20') AND SUBSTR(avt.stage_best,1,1)='1' AND colorec_hes=1 AND tc.tumour_flag=0 THEN colohes.hessg_trust_code 
END AS colohessg_trust_code 
,CASE WHEN avt.site_icd10_o2 IN ('C181') AND colorec_avtreat_appen=1 THEN coloavt_appen.avsg_trust_code 
END AS appenavsg_trust_code 
, CASE WHEN avt.site_icd10_o2 IN ('C181') AND colorec_hes_appen=1 AND tc.tumour_flag=0 THEN colohes_appen.hessg_trust_code 
END AS appenhessg_trust_code

---------------bladder--------------------------------- 
,CASE WHEN avt.site_icd10_o2_3char IN ('C67') AND SUBSTR(avt.t_best, 1,1) = '1' AND bladder_avtreat=1 THEN blad1_avt.avsg_trust_code 
END AS bladavsg_trust_code 
, CASE WHEN avt.site_icd10_o2_3char IN ('C67') AND SUBSTR(avt.t_best, 1,1) = '1' AND bladder_hes=1 AND tc.tumour_flag=0 THEN blad1_hes.hessg_trust_code 
END AS bladhessg_trust_code 

,CASE WHEN avt.site_icd10_o2 IN ('D090') AND avt.morph_icd10_o2 = '8130' AND bladder_avtreat=1 THEN blad1_avt.avsg_trust_code 
END AS blad_insitu_avsg_trust_code 
, CASE WHEN avt.site_icd10_o2 IN ('D090') AND avt.morph_icd10_o2 = '8130' AND bladder_hes=1 AND tc.tumour_flag=0 THEN blad1_hes.hessg_trust_code 
END AS blad_insitu_hessg_trust_code 

---------------liver--------------------------------- 
,CASE WHEN avt.site_icd10_o2_3char IN ('C22') AND SUBSTR(avt.stage_best,1,1)='1' AND liver_avtreat=1 THEN livavt.avsg_trust_code 
END AS livavsg_trust_code 
, CASE WHEN avt.site_icd10_o2_3char IN ('C22') AND SUBSTR(avt.stage_best,1,1)='1' AND liver_hes=1 AND tc.tumour_flag=0 THEN livhes.hessg_trust_code 
END AS livhessg_trust_code 

---------------oesophageal--------------------------------- 
,CASE WHEN avt.site_icd10_o2_3char IN ('C15') AND SUBSTR(avt.stage_best,1,2)='1A' AND oesoph_avtreat=1 THEN oesoavt.avsg_trust_code 
END AS oesoavsg_trust_code 
, CASE WHEN avt.site_icd10_o2_3char IN ('C15') AND SUBSTR(avt.stage_best,1,2)='1A' AND oesoph_hes=1 AND tc.tumour_flag=0 THEN oesohes.hessg_trust_code 
END AS oesohessg_trust_code 

---------------stomach--------------------------------- 
, CASE WHEN avt.site_icd10_o2_3char IN ('C16') AND SUBSTR(avt.stage_best,1,2)='1A' AND stomach_avtreat=1 THEN stomavt.avsg_trust_code 
END AS stomavsg_trust_code 
, CASE WHEN avt.site_icd10_o2_3char IN ('C16') AND SUBSTR(avt.stage_best,1,2)='1A' AND stomach_hes=1 AND tc.tumour_flag=0 THEN stomhes.hessg_trust_code 
...
FROM av2020.at_tumour_england@casref01 AVT 
INNER JOIN username.tr_tumour_cohort@casref01 tc ON avt. tumourid =tc. tumourid
LEFT JOIN username.tr_av_ct@casref01 avct ON avt.tumourid=avct.tumourid 
LEFT JOIN username.tr_sact@casref01 sact ON avt.tumourid=sact.tumourid 
LEFT JOIN username.tr_sact_2@casref01 sact2 ON avt.tumourid=sact2.tumourid
LEFT JOIN username.tr_av_rt@casref01 avrt ON avt.tumourid=avrt.tumourid 
LEFT JOIN username.tr_av_sg@casref01 avsg ON avt.tumourid=avsg.tumourid AND (tc.tumour_code NOT IN ('C48OTHER')) 
LEFT JOIN username.tr_rtds@casref01 rtds ON avt.tumourid=rtds.tumourid 
LEFT JOIN username.tr_hes_sg@casref01 hessg ON avt.tumourid=hessg.tumourid AND (tc.tumour_code NOT IN ('C48OTHER')) 
LEFT JOIN username.tr_rtds_2@casref01 rtds2 ON avt.tumourid=rtds2.tumourid 

-- Add further joins for stage-specific resections:

---------------CERVICAL------------------
LEFT JOIN username.tr_av_conebiops@casref01 CBAVT ON avt.tumourid=cbavt.tumourid 
LEFT JOIN username.tr_hes_conebiops@casref01 CBhes ON avt.tumourid=cbhes.tumourid 
LEFT JOIN username.tr_av_lymph@casref01 lyavt ON avt.tumourid=lyavt.tumourid 
LEFT JOIN username.tr_hes_lymph@casref01 lyhes ON avt.tumourid=lyhes.tumourid 

---------------Colorectal---------------------------------
LEFT JOIN username.tr_av_colorec@casref01 coloavt ON avt.tumourid=coloavt.tumourid 
LEFT JOIN username.tr_hes_colorec@casref01 colohes ON avt.tumourid=colohes.tumourid 
LEFT JOIN username.tr_av_coloappen@casref01 coloavt_appen ON avt.tumourid=coloavt_appen.tumourid 
LEFT JOIN username.tr_hes_coloappen@casref01 colohes_appen ON avt.tumourid=colohes_appen.tumourid 

---------------Bladder---------------------------------
LEFT JOIN username.tr_av_bladder@casref01 blad1_avt ON avt.tumourid=blad1_avt.tumourid 
LEFT JOIN username.tr_hes_bladder@casref01 blad1_hes ON avt.tumourid=blad1_hes.tumourid 

---------------Liver---------------------------------
LEFT JOIN username.tr_av_liver@casref01 livavt ON avt.tumourid=livavt.tumourid 
LEFT JOIN username.tr_hes_liver@casref01 livhes ON avt.tumourid=livhes.tumourid 

---------------Oesophageal---------------------------------
LEFT JOIN username.tr_av_oesoph@casref01 oesoavt ON avt.tumourid=oesoavt.tumourid 
LEFT JOIN username.tr_hes_oesoph@casref01 oesohes ON avt.tumourid=oesohes.tumourid 

---------------Stomach---------------------------------
LEFT JOIN username.tr_av_stomach@casref01 stomavt ON avt.tumourid=stomavt.tumourid 
LEFT JOIN username.tr_hes_stomach@casref01 stomhes ON avt.tumourid=stomhes.tumourid 

-- Additional demographics 
LEFT JOIN av2020.at_geography_england@casref01 atg ON avt.tumourid=atg.tumourid --join on tumour id
...
LEFT JOIN av2020.charlson_2006to2020@casref01 chrl ON chrl.tumourid=avt.tumourid

/*
LEFT JOIN (select avtu.tumourid
            , CASE WHEN avtu.stage_best is null THEN 'X' 
               WHEN (SUBSTR(avtu.stage_best,1,1) NOT IN ('1','2','3','4'))  THEN  'X' 
               ELSE SUBSTR(avtu.stage_best,1,1) END AS stage 
            from av2020.at_tumour_england@casref01 avtu 
            WHERE avtu.diagnosisyear BETWEEN 2012 AND 2020
            AND (NOT (avtu.site_icd10_o2_3char='C50' AND SUBSTR(avtu.stage_best,1,1)='0') or avtu.stage_pi is null) 
) stage_nopagets 
ON stage_nopagets.tumourid=avt.tumourid 
*/
;

grant select on av_treatment_table_1320_4p8 to 'other_user';
grant select on opcs4resection_lookup_13_20 to 'other_user';
grant select on timeframe_lookup_13_20 to 'other_user';
grant select on tr_tumour_cohort to 'other_user';
